---
layout: default
title: Team Meeting 2 Agenda
---

[← Back to Home](index.html)

## Team Presentation Preparation

**Important:** Only the Notetaker uses a laptop to type during the meeting. All other team members should verbally share their thoughts with the Notetaker, who will record them.

| Item | Details |
|------|---------|
| Meeting Date |  |
| Meeting Time |  |
| Meeting Location |  |
| Team Number |  |

### Present Students

| Role | Name |
|------|------|
| Notetaker |  |
| Member |  |
| Member |  |
| Member |  |
| Member |  |
| Member |  |

### Absent Students

| Name | Submitted Later (Y/N) |
|------|-----------------------|
|  |  |
|  |  |

> **Note:** If you were absent, you can still receive points by obtaining the meeting notes from your team, adding your own reflections, and submitting to Canvas by Sunday 11:59 PM.

---

## Purpose of This Week's Meeting

Prepare for your team presentation by confirming your assigned chapter, reviewing the rubric, and creating your presentation slides.

---

## Instructions for Notetaker

The Notetaker is responsible for:
1. Facilitating the discussion and ensuring all team members contribute
2. Managing time during the meeting
3. Recording key points from the discussion
4. Collecting all survey completion codes from team members
5. Downloading the document as a Word file and submitting to Canvas

**Before the meeting:** Open today's team meeting agenda and make a copy for your team.

---

## Meeting Agenda

### [5 minutes] Confirm Your Team's Presentation Assignment

Look at the syllabus together and find your team's assigned chapter and presentation date.

**Tip:** All team members should check on their smartphone or laptop. If someone cannot find it, help them locate it in the syllabus.

| Item | Details |
|------|---------|
| Assigned Chapter |  |
| Chapter Topic |  |
| Presentation Date |  |
| Case Study Title |  |
| Case Study Link |  |

---

### [10 minutes] Review the Team Presentation Rubric

Open the [Team Presentation Rubric](team-presentation-rubric) in the syllabus and review it together as a team. Discuss each evaluation criteria.

**Key sections to review:**

- Preparation (24 points)
- Content and Slide Quality (24 points)
- Presentation (24 points)
- Interaction with the Audience (24 points)
- Discussion Facilitation (24 points)

**Note-taking tip:** Team members share their thoughts freely, and the Notetaker records them below. The Notetaker can also share and write their own thoughts. Keep it brief during the meeting (use short keywords). You can use AI to polish your notes after the meeting.

| Name | What stood out to you from the rubric? |
|------|----------------------------------------|
| 1. |  |
| 2. |  |
| 3. |  |
| 4. |  |
| 5. |  |
| 6. |  |

---

### [5 minutes] Review the Case Study

Find your assigned case study in the syllabus and open it. You only have 5 minutes, so just quickly skim through the case to get a general idea.

**Discussion Questions:**

| Question | Your Team's Notes |
|----------|-------------------|
| What is the main issue in this case? |  |
| What key concepts from the chapter relate to this case? |  |
| What discussion questions will engage the audience? |  |

> **Important:** The case study includes discussion questions at the end, but they can be difficult for the audience to answer without reading the full case. Prepare debate-style questions that students can answer from employee vs. employer perspectives, even without detailed knowledge of the case. See the Team Presentation Rubric for more guidance.

---

### [10 minutes] Choose Your Presentation Tool and Create Slides

As a team, decide which online presentation tool you will use to create your slides.

**Options:**

- Google Slides (Recommended - easy to collaborate)
- Microsoft PowerPoint Online
- Canva
- Other: ________________

| Item | Details |
|------|---------|
| Chosen Tool |  |
| Who will create the initial slide deck? |  |

**Action Items:**
1. Create a new blank presentation (just create an empty slide deck for now - you will add content later)
2. Share the link with all team members so everyone can edit
3. Add the slide link to the syllabus (next to your assigned case)

---

### [5 minutes] Add Your Slide Link to the Syllabus

Follow these steps to add your slide link to the Google Document syllabus:

1. Copy your slide link
2. Open the syllabus (Google Document)
3. Find your assigned case (e.g., "Ch 3 Case [Link] [Slide]")
4. Click on "[Slide]" and add your link

**Need help adding a link?** Watch this tutorial: [How to Add a Link in Google Docs](https://youtu.be/_cnke4qwY6I?si=xI34R_kPnmoAaVPp&t=57)

| Checklist | Done (Y/N) |
|-----------|------------|
| Slide link created |  |
| Link added to syllabus |  |
| All team members can edit the slides |  |

---

### [5 minutes] Set Sharing Permissions (Very Important!)

Make sure your slides are accessible to everyone, including the instructor.

**Steps:**
1. Open your slide in an **incognito/private browser window**
2. If you can view it without signing in, your settings are correct
3. If you need to sign in or request access, update sharing settings to "Anyone with the link can view"

> **Warning:** If the instructor cannot open your slides during class, points will be deducted.

| Checklist | Done (Y/N) |
|-----------|------------|
| Tested link in incognito window |  |
| Link works without sign-in |  |

**Your Slide Link:** ________________________________

---

### [1 minute] Wrap-up

Check the syllabus to find out when the next team meeting will be. Discuss who will be the next Notetaker.

**Our next meeting:**

| Item | Details |
|------|---------|
| Date |  |
| Time |  |
| Meeting Location |  |
| Notetaker |  |

---

### [5 minutes] Self and Peer Review Evaluation Survey

Conduct a survey for self/peer evaluation. You can find the URL in the syllabus.

**Tip:** Bookmark this survey URL on your phone. You will use it every week for team meetings.

Once you finish the survey, you will receive the survey completion code. The Notetaker needs to collect all survey completion codes from team members. The completion code will be used as an attendance check.

| Name | Survey Completion Code |
|------|----------------------|
| 1. |  |
| 2. |  |
| 3. |  |
| 4. |  |
| 5. |  |
| 6. |  |
